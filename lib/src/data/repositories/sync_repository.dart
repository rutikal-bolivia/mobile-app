...
class SyncRepository {
  ...
  Future<bool> synchronize() async {
    try {
      ...
      debugPrint('[SYNC] Cursor local: $localCursor | Versión remota: $remoteVersion');
      ...
      while (hasMore) {
        ...
        if (response.statusCode != 200) {
          throw Exception('Error al descargar cambios: Código ${response.statusCode}');
        }
        ...
        final int pageCursor = int.tryParse(data['cursor'].toString()) ?? cursor;
        ...
        if (changesList.isNotEmpty) {
          ...
          await db.insert(
            'sync_meta',
            {'clave': 'version', 'valor': pageCursor.toString()},
  ...
  Future<void> _processChangesPageTransactionally(List<dynamic> changes, int pageCursor) async {
    final db = await dbService.database;
  ...
  Future<void> _applyUpsert(
    ...
    if (tableName == 'paradas') {
      ...
      await dbExecutor.insert(tableName, row, conflictAlgorithm: ConflictAlgorithm.replace);

      // Re-sincronizar relaciones de la tabla puente ruta_horario
      await dbExecutor.delete('ruta_horario', where: 'horario_id = ?', whereArgs: [horarioId]);
        ...
        for (final routeId in routeIds) {
          ...
          if (rId != 0) {
            await dbExecutor.insert(
              'ruta_horario',
              {'ruta_id': rId, 'horario_id': horarioId},
              conflictAlgorithm: ConflictAlgorithm.replace,
            );
          }
        }
